...
fs.readdir("src", (_,items)=>{
	...
	let emcc = `emcc ${srcs} -s WASM=1 -s TOTAL_MEMORY=250MB -s ALLOW_MEMORY_GROWTH=1 -s USE_PTHREADS=1 -s PTHREAD_POOL_SIZE=2 -D WASM ${optimizeFlag}-o wasmbuild/cengine.html -s EXPORTED_FUNCTIONS="[${EXPORTED_FUNCTIONS.map(ef => "'_"+ef+"'").join(",")}]" -s EXTRA_EXPORTED_RUNTIME_METHODS="['cwrap']"`
