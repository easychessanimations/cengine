...
fs.readdir("src", (_,items)=>{
	...
	let emcc = `emcc ${srcs} -s WASM=1 -s TOTAL_MEMORY=30MB -s ALLOW_MEMORY_GROWTH=1 -s USE_PTHREADS=0 -s PTHREAD_POOL_SIZE=1 -D WASM ${optimizeFlag}-o wasmbuild/cengine.html -s EXPORTED_FUNCTIONS="[${EXPORTED_FUNCTIONS.map(ef => "'_"+ef+"'").join(",")}]" -s EXTRA_EXPORTED_RUNTIME_METHODS="['cwrap']"`
